...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 0, axis=0)] = Road
    """
...
def rgb2label(label):
    ...
    label_seg[np.all(label == Tree, axis=-1)] = 1
    label_seg[np.all(label == LowVeg, axis=-1)] = 1
    label_seg[np.all(label == Moving_Car, axis=-1)] = 0
    label_seg[np.all(label == Static_Car, axis=-1)] = 1
